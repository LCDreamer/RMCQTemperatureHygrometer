...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Defaults
        _maximumValue = 150.0f;//滑竿的最大值
        _minimumValue = 0.0f;//滑竿的最小值
        _currentValue = 0.0f;
        _lineWidth = 10;
        _unfilledColor = [UIColor blackColor];//开始的颜色黑色
        _filledColor = [UIColor redColor];//滑动过后的红色
...
//
			if ( [[UIDevice currentDevice].systemVersion doubleValue]>=7.0) {
				...
				[label drawInRect:labelLocation withFont:[UIFont systemFontOfSize:10.0f]];
				
			}
        }
    }
}
-(void)humidityConvertedToCircleForCoordinatepoints:(float)humidity{
	CGPoint circleCenter;
	circleCenter.x=self.frame.size.width/2;
	circleCenter.y=self.frame.size.height/2;
	float corner=(360*humidity)/Maximum_Value;
	CGPoint point;
	if ( corner>=0 && corner<=90) {
		double sinVelueX=(corner*(M_PI/2))/90;
		point.x=circleCenter.x+radius*sin(sinVelueX);
		point.y =circleCenter.y-radius*cos(sinVelueX);
		
	}else if (corner>90 && corner<=180){
		double sinVelueX=((corner-90)*(M_PI/2))/90;
		point.x =circleCenter.x+radius*cos(sinVelueX);
		point.y =circleCenter.y+radius*sin(sinVelueX);
		
	}else if (corner>180 && corner<=270){
		double sinVelueX=((corner-180)*(M_PI/2))/90;
		point.x =circleCenter.x-radius*sin(sinVelueX);
		point.y =circleCenter.y+radius*cos(sinVelueX);
		
	}else if (corner>270 && corner<=360){
		double sinVelueX=((corner-270)*(M_PI/2))/90;
		point.x =circleCenter.x-radius*cos(sinVelueX);
		point.y =circleCenter.y-radius*sin(sinVelueX);
	}
